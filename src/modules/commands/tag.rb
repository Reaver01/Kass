    ...
    module Tag
      ...
      command(
        ...
        if args[0] == 'create'
          ...
          tag_user = BOT.parse_mention(args[1])
          tag_user = event.user if tag_user.nil?
          event << "Here are the tags belonging to **#{tag_user.name}**:"
          Database::Tag.each { |tag| event << "`#{tag.tag_name}`" if tag_user.id == tag.owner_id }
          nil
        else
